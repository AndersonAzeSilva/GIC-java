...
public class ImportacaoExcelService {
    ...
    public static ResultSet buscarTodas() throws Exception {
        ...
        String sql
                ...
                + "FROM " + DBTables.TBL_IMPORTACOES_EXCEL + " ORDER BY data_importacao DESC";
    ...
    public static ResultSet buscar(String usuario, Date inicio, Date fim) throws Exception {
        ...
        if (usuario != null && !usuario.trim().isEmpty()) {
            pst.setString(idx++, "%" + usuario.trim() + "%");
        }
        if (inicio != null) {
            pst.setTimestamp(idx++, new Timestamp(inicio.getTime()));
        }
        if (fim != null) {
            pst.setTimestamp(idx++, new Timestamp(fim.getTime()));
        }
